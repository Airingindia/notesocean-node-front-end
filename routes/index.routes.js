...
router.get("/google-signin", (req, res, next) => {
	res.redirect(
		Buffer.from(req.cookies.api, "base64").toString("ascii") +
		...
		profileControllers
			...
			.then((userInfo) => {
				if (userInfo.profileImage == null) {
					...
					userInfo.profileImage.indexOf("'https://profiles.ncdn.in") == -1
				) {
					userInfo.profileImage = userInfo.profileImage.replace(
						"https://profiles.ncdn.in",
						"https://profiles.ncdn.in/fit-in/100x100"
					);
				}
...
router.get("/collections/:collecton_id", async (req, res, next) => {
	...
	collectionController
		.getCollectionDetails(collecton_id, req)
		.then((collection) => {
			var jsonld = [];
