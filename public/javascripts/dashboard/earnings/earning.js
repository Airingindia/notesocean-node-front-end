$(document).ready(function () {
	...
	app
		.getUserEarning()
		.then((data) => {
			$(".current-balance-amount").html(
				"&#x20B9; " +
					Number(data?.totalEarning ? data.totalEarning : 0).toFixed(2)
			);
			$(".avl-balance").html(
				"&#x20B9; " +
					Number(data?.totalEarning ? data.totalEarning : 0).toFixed(2)
	...
	$.ajax({
		...
		success: function (data) {
            ...
            <table class="table table-striped table-borderless table-hover withdrawal-table withdrawal-table-styling">
  ...
  <tbody>
    <tr>
      ...
      <td> ${amount}</td>
      <td>${payoutid}</td>
      <td>${moment.unix(date).format("DD MMM YYYY")}</td>
      <td>${status}</td>
      <td ${
				status == "issued" || status == "processing" || status == "pending"
					? `<a href="/dashboard/earning/${payoutid}" class="withdrawal-status-check">Check Status</a>`
					: ""
			}</td>
    </tr>
